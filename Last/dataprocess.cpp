...
static void add_friend(int SSID1, int SSID2, vector<Student>& list) { // thêm SSID2 là bạn của SSID1 và ngược lại
	list[getord(SSID1, list)].addfriendcode(SSID2);
	list[getord(SSID2, list)].addfriendcode(SSID1);

}
static void delete_friend(int SSID1, int SSID2, vector<Student>& list) { // xóa SSID2 khỏi danh sách bạn của SSID1 và ngược lại
	list[getord(SSID1, list)].deletefriendcode(SSID2);
	list[getord(SSID2, list)].deletefriendcode(SSID1);
}
...
static void print_full(vector<Student> stdu, vector<string> hbby, vector<string> hbit) { //in full thông tin của student
